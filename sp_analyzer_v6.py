...
def split_statements(sql):
    """
    Split cleaned SQL body into individual DML statements.
    This is the key to fixing alias collision — aliases are
    resolved per-statement, not globally across the whole SP.
    """
...
SKIP_WORDS = {
    ...
    'WHEN','THEN','MATCHED','BY'
}

# ── Table ref parser ──────────────────────────────────────────────────────────
def parse_table_ref(raw):
    """Returns (schema_upper, base_upper, display_full)."""
...
# ── Per-statement Alias Map ───────────────────────────────────────────────────
def build_alias_map(stmt):
    """
    Build alias→full_key map for ONE statement only.
    Handles:
      FROM   tbl t
      FROM   tbl AS t
      JOIN   tbl t
      JOIN   tbl AS t
      UPDATE tbl t SET ...
      MERGE  tbl AS t USING ...
    Returns dict: alias_upper → full_key_upper
    """
    alias_map = {}
    pat = re.compile(
    ...
    for m in pat.finditer(stmt):
        ...
        if alias == base:          # alias == table name itself, not a real alias
            continue
...
# Table-level patterns with their operation type
TABLE_OP_PATTERNS = [
...
def extract_all(sql):
    ...
    # ── Step 1: Collect CTE names AND map each CTE to its source physical table
    cte_names = set()
    # cte_to_physical: CTE_NAME_UPPER → full_physical_key_upper
    # When a CTE selects from exactly one physical table, columns via that CTE
    # alias can be resolved back to the physical table.
    cte_to_physical = {}

    cte_blocks = list(re.finditer(
        r'(?:(?:^|,)\s*)([\w\[\]]+)\s+AS\s*\(\s*SELECT(.*?)FROM\s+((?:[\w\[\]]+\.)*[\w\[\]]+)',
        clean, re.IGNORECASE | re.DOTALL))
    for m in cte_blocks:
        cname = strip_name(m.group(1)).upper()
        src   = m.group(3).strip()
        schema, base, full = parse_table_ref(src)
        cte_names.add(cname)
        if base not in SKIP_WORDS and not is_temp(src) and not is_var(src):
            cte_to_physical[cname] = full

    # also catch WITH name AS pattern at start
    for m in re.finditer(r'\bWITH\b\s+([\w\[\]]+)\s+AS\s*\(', clean, re.IGNORECASE):
        cte_names.add(strip_name(m.group(1)).upper())

    # physical: full_key_upper → {schema, base, ops, aliases, columns}
    physical = {}
    ...
    # ── Step 2: Split into individual DML statements
    #    This is the alias collision fix — each statement gets its OWN alias map
    ...
    for stmt in statements:
        ...
        # Register aliases back onto physical table entries (for display)
        for alias, full_key in alias_map.items():
            schema, base, _ = parse_table_ref(full_key)
            if full_key in physical:
                physical[full_key]["aliases"].add(alias)

        # Extract physical tables for this statement
        for pat, op in TABLE_OP_PATTERNS:
            for m in re.finditer(pat, stmt, re.IGNORECASE):
                raw = m.group(1).strip()
                schema, base, full = parse_table_ref(raw)
                if (base in SKIP_WORDS or base in cte_names
                        or is_temp(raw) or is_var(raw) or len(base) < 2):
                    continue
        ...
        # ── Column extraction — qualified (alias.col or table.col)
        for m in re.finditer(r'\b([\w\[\]]+)\.([\w\[\]]+)\b', stmt):
            prefix = strip_name(m.group(1)).upper()
            col    = strip_name(m.group(2)).upper()
            if (col in SKIP_WORDS or is_var(col)
                    or not re.match(r'^[A-Z_][A-Z0-9_]*$', col)):
                continue

# ── Collect physical tables referenced in THIS statement (excl. CTEs/temp)
        stmt_tables = []
        for pat, _ in TABLE_OP_PATTERNS:
            for m in re.finditer(pat, stmt, re.IGNORECASE):
                raw = m.group(1).strip()
                schema, base, full = parse_table_ref(raw)
                if (base in SKIP_WORDS or base in cte_names
        ...
        def assign_col(col, stmt_tables, alias_map):
            """
            Assign an unqualified column to a table using these rules:
            1. If only ONE physical table in this statement → assign directly.
            2. If multiple tables but only ONE is an UPDATE/INSERT target → assign there.
            3. Otherwise → unresolved.
            """
            if not col or col in SKIP_WORDS or not re.match(r'^[A-Z_][A-Z0-9_]*$', col):
                return
            if len(stmt_tables) == 1:
                key = stmt_tables[0]
                if key in physical:
                    physical[key]["columns"].add(col)
                    return
            # fallback → unresolved
            physical.setdefault('__UNRESOLVED__', {
                "schema": "", "base": "__UNRESOLVED__",
                "ops": set(), "aliases": set(), "columns": set()
            })
            physical['__UNRESOLVED__']["columns"].add(col)

        # ── SELECT list unqualified columns
        for block_m in re.finditer(r'\bSELECT\b(.*?)\bFROM\b', stmt, re.IGNORECASE | re.DOTALL):
            ...
            # Strip AS aliases: "col AS 'Label'" → keep col, drop label
            block = re.sub(r'\bAS\s+[\'"\[]?[\w\s]+[\'"\]]?', '', block, flags=re.IGNORECASE)
            for token in re.split(r'[,\s]+', block):
                token = strip_name(token.split('.')[-1]).upper()
                if token and token != '*':
                    assign_col(token, stmt_tables, alias_map)

        # ── WHERE unqualified col = val
        for m in re.finditer(r'\bWHERE\b(.*?)(?:\bGROUP\b|\bORDER\b|\bHAVING\b|$)',
                              stmt, re.IGNORECASE | re.DOTALL):
            where_block = m.group(1)
            for wm in re.finditer(r'\b([\w\[\]]+)\s*[=<>!]', where_block):
                col = strip_name(wm.group(1)).upper()
                assign_col(col, stmt_tables, alias_map)
        ...
        for m in re.finditer(
            ...
            for c in m.group(1).split(','):
                ...
                assign_col(c, stmt_tables, alias_map)

    # ── Step 3: Clean unresolved — remove cols already mapped to real tables
    mapped = set()
    ...
    # ── Dynamic SQL flag
    dynamic = bool(re.search(
        r'EXEC\s*\(|EXECUTE\s*\(|sp_executesql', clean, re.IGNORECASE))

    return physical, dynamic
...
def build_excel(all_results, output_path):
    ...
    for ri, (pname, source, dialect, physical, dynamic) in enumerate(all_results, 1):
    ...
    row = 2
    for pname, source, dialect, physical, dynamic in all_results:
    ...
    for pname, source, dialect, physical, dynamic in all_results:
        for key, info in sorted(
            ...
            if not cols:
                vals = [pname, schema, base, '(no columns resolved)',
                        'alias.col / table.col', ops]
                for ci, v in enumerate(vals, 1):
        ...
        # Unresolved columns (table not determinable)
        unres_info = physical.get('__UNRESOLVED__')
        if unres_info:
            for col in sorted(unres_info["columns"]):
                vals = [pname, '—', '(table not determined)',
                        col, 'unqualified token', '—']
                for ci, v in enumerate(vals, 1):
                    c = wc.cell(row=row, column=ci, value=v)
                    style_data(c, row)
                    if ci in [2, 3, 5, 6]:
    ...
    hdrs = ["Schema", "Table Name", "Column Name",
    ...
    schema_map = defaultdict(lambda: defaultdict(
        ...
        for tname in sorted(schema_map[schema].keys()):
            ...
            if not cols:
                ...
                ws4.cell(row=row, column=4).font = Font(
                    bold=True, color=op_color(tinfo["ops"]),
                    name="Arial", size=10)
                row += 1
            else:
    ...
    notes = [
        ...
        ("Alias Collision", "Aliases resolved per-statement — same alias in different statements mapped correctly"),
        ("Unresolved Cols", "Column found in SQL but table could not be determined from context"),
        ("Dynamic SQL ⚠",  "EXEC() / sp_executesql contents cannot be statically analyzed — review manually"),
...
# ── Safe File Reader (handles any Windows/SSMS encoding) ─────────────────────
def read_file_safe(filepath):
    """
    Try encodings in order: utf-8-sig (BOM), utf-8, windows-1252, latin-1.
    latin-1 never fails — it is the guaranteed fallback.
    Also strips the BOM character if present.
    """
    encodings = ["utf-8-sig", "utf-8", "windows-1252", "cp1252", "latin-1"]
    for enc in encodings:
        try:
            text = Path(filepath).read_text(encoding=enc)
            # Strip BOM just in case
            text = text.lstrip('\ufeff')
            print(f"  [encoding] {Path(filepath).name} → read as {enc}")
            return text
        except (UnicodeDecodeError, LookupError):
            continue
    # Should never reach here because latin-1 accepts all bytes
...
def process_sql(sql, source, forced_dialect):
    ...
    for pname, body in procs:
        physical, dynamic = extract_all(body)
        phys = {k: v for k, v in physical.items() if k != '__UNRESOLVED__'}
        src  = Path(source).name if source != "stdin" else "pasted"
        print(f"  [{src}] → {pname}: {len(phys)} physical table(s), "
              f"dialect={dialect}"
              + (" ⚠ dynamic SQL" if dynamic else ""))
        results.append((pname, source, dialect, physical, dynamic))
    return results
